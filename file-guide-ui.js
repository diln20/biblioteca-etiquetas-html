(()=>{
  ...
  const guidedCourse=item=>/Código\s+(?:Angular|React|Vue|Svelte|Solid\.js|FastAPI|Django REST|Django Framework)/i.test(String(item?.codeLabel||''))
    || /^(?:Angular|React|Vue|Svelte|Solid\.js|FastAPI|Django REST|Django Framework)$/i.test(String(item?.kind||''));
