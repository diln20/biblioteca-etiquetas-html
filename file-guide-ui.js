(()=>{
  ...
  const guidedCourse=item=>/Código\s+(?:Angular|React|Vue|Svelte|Solid\.js|FastAPI|Django REST|Django Framework|Oracle|PostgreSQL|MongoDB|Neo4j|Redis|Cassandra|Fundamentos|Comparativa|Herramientas)/i.test(String(item?.codeLabel||''))
    || /^(?:Angular|React|Vue|Svelte|Solid\.js|FastAPI|Django REST|Django Framework|Base de datos|Herramientas)$/i.test(String(item?.kind||''));
