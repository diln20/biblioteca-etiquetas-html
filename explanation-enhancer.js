(()=>{
  ...
  const detailFor=(section,item)=>{
    ...
    if(item.tag==='Construcción guiada')return `Sigue la lista “Dónde colocar cada código” en orden. Cada bloque indica si debes crear, reemplazar o ampliar el archivo; guarda y comprueba el resultado antes de continuar.`;
    ...
    if(section.title.startsWith('Frameworks frontend'))return `Sigue la progresión de «${label}»: identifica el comando o concepto, localiza el componente y su estado en el código, y compáralos con el HTML que finalmente aparece en el resultado. Practica cada paso dentro de un proyecto creado para ese framework.`;
    if(section.title.startsWith('Frameworks CSS'))return `Sigue la progresión de «${label}»: identifica cómo se carga el framework, relaciona cada clase con el elemento HTML que recibe el estilo y compara las variantes responsive con el resultado. Repite el ejemplo dentro de un proyecto separado para evitar conflictos entre frameworks.`;
    if(section.title.startsWith('Proyectos con frameworks'))return `Ubica «${label}» dentro del proyecto completo: distingue qué parte corresponde al framework web, qué clases aporta el framework CSS y cómo ambos producen el resultado. Sigue el orden de preparación, componentes, datos, responsive y verificación sin saltar directamente al código final.`;
    if(section.title.startsWith('Backend FastAPI'))return `Sigue «${label}» desde la solicitud HTTP hasta la respuesta: identifica la validación de FastAPI, la regla de negocio, la sesión de SQLAlchemy y el efecto en PostgreSQL. Relaciónalo con la tienda de Ana: consultar productos, registrar mercancía o confirmar una venta. Ejecuta primero el caso correcto y después prueba validación, ausencia, conflicto y rollback.`;
