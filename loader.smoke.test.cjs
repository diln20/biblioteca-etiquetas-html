...
setTimeout(() => {
  ...
  const result = {
    ...
    practiceExpansionScript: written.includes('practice-expansion.js?v=2'),
    ...
    courseUiScript: written.includes('course-ui.js?v=14'),
    ...
    divManipulation: sections.find(section => section.title === 'HTML · Manipulación de DIV')?.items.length === 12 && sections.findIndex(section => section.title === 'HTML · Manipulación de DIV') + 1 === sections.findIndex(section => section.title === 'CSS · Principiante') && ['HTML','CSS','JavaScript'].every(term => sections.find(section => section.title === 'HTML · Manipulación de DIV')?.items.some(item => item.code.includes(term))),
