(()=>{
  ...
  sections.push({
    ...
    items:[
      ...
      L(
        'pokeapi html javascript',
        '3. HTML + JavaScript · buscador de Pokémon con PokéAPI',
        'Ahora la API deja de vivir solo en la consola. El HTML aporta un input, un botón y una zona de resultado; JavaScript lee el nombre o número del Pokémon, consulta PokéAPI y actualiza el DOM. PokéAPI no requiere autenticación para su API REST pública, por lo que es apropiada para practicar fetch directamente desde el navegador.',
        `<!-- index.html -->\n<!DOCTYPE html>\n<html lang="es">\n<head>\n  <meta charset="UTF-8">\n  <meta name="viewport" content="width=device-width, initial-scale=1.0">\n  <title>Buscador de Pokémon</title>\n</head>\n<body>\n  <label for="pokemon">Pokémon</label>\n  <input id="pokemon" value="pikachu" placeholder="pikachu o 25">\n  <button id="buscar" type="button">Buscar</button>\n  <div id="resultado">Escribe un nombre o número.</div>\n\n  <script src="app.js" defer><\/script>\n</body>\n</html>\n\n// app.js\nconst input = document.querySelector("#pokemon");\nconst button = document.querySelector("#buscar");\nconst result = document.querySelector("#resultado");\n\nbutton.addEventListener("click", buscarPokemon);\ninput.addEventListener("keydown", function(event) {\n  if (event.key === "Enter") buscarPokemon();\n});\n\nasync function buscarPokemon() {\n  const name = input.value.trim().toLowerCase();\n\n  if (!name) {\n    result.textContent = "Escribe un nombre o número.";\n    return;\n  }\n\n  result.textContent = "Cargando...";\n\n  try {\n    const response = await fetch(\n      \`https://pokeapi.co/api/v2/pokemon/\${encodeURIComponent(name)}\`\n    );\n\n    if (!response.ok) {\n      throw new Error("Pokémon no encontrado");\n    }\n\n    const pokemon = await response.json();\n    const types = pokemon.types\n      .map(function(item) { return item.type.name; })\n      .join(", ");\n\n    result.replaceChildren();\n\n    const title = document.createElement("h2");\n    title.textContent = pokemon.name;\n\n    const image = document.createElement("img");\n    image.src = pokemon.sprites.front_default;\n    image.alt = pokemon.name;\n    image.width = 160;\n    image.height = 160;\n\n    const idP = document.createElement("p");\n    idP.textContent = \`ID: \${pokemon.id}\`;\n\n    const typeP = document.createElement("p");\n    typeP.textContent = \`Tipo: \${types}\`;\n\n    const sizeP = document.createElement("p");\n    sizeP.textContent =\n      \`Altura: \${pokemon.height / 10} m · Peso: \${pokemon.weight / 10} kg\`;\n\n    result.append(title, image, idP, typeP, sizeP);\n  } catch (error) {\n    result.textContent = error.message;\n    console.error(error);\n  }\n}`,
        preview('Resultado esperado','<div><strong>Pikachu</strong><p style="margin:6px 0 0">Imagen · ID · tipo · altura · peso</p></div>'),
        'El HTML define dónde se muestra la información; JavaScript transforma el JSON recibido en elementos visibles del DOM.',
        {
          guide:[['Crear','javascript/public-apis/pokemon/index.html','Crea el input, botón, resultado y carga app.js con defer.'],['Crear','javascript/public-apis/pokemon/app.js','Evento, función async, fetch, validación y render del Pokémon.']],
          filesToCreate:[file('javascript/public-apis/pokemon/index.html','Interfaz HTML completa.'),file('javascript/public-apis/pokemon/app.js','Lógica Fetch y DOM con PokéAPI.')],
          exerciseTitle:'Ejercicio para ti · mejora el buscador',
          exerciseTasks:['Prueba búsquedas como pikachu, charizard y 1.','Muestra también una habilidad usando pokemon.abilities.','Permite buscar pulsando Enter.','Prueba un nombre inexistente y conserva un mensaje de error claro.'],
          exerciseExtra:'Agrega un botón Limpiar que borre el input, la imagen y el resultado.'
        }
      ),
      L(
